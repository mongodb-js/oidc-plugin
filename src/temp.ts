...
describe('OIDC plugin (local OIDC provider)', function () {
  ...
  context.skip('with functioning auth code flow', function () {
  ...
  context.skip('when the user aborts an auth code flow', function () {
    beforeEach(function () {
  ...
  context.skip('with functioning device auth flow', function () {
    beforeEach(function () {
      plugin = createMongoDBOIDCPlugin({
        ...defaultOpts,
    ...
    it('can request tokens without the browser', async function () {
      const result = await requestToken(
        plugin,
        provider.getMongodbOIDCDBInfo()
      );
  ...
  context('with functioning device auth flow - no id_token', function () {
    it('updateStateWithTokenSet - handles missing id_token scenario with a warning', function () {
      const customLogger = new EventEmitter();
      const spy = sinon.spy(logger, 'emit');
      const plugin = new MongoDBOIDCPluginImpl({});
      const state: UserOIDCAuthState = {
        serverOIDCMetadata: { issuer: '123', clientId: 'testClient' },
        ...
        currentTokenSet: {
          ...
          tryRefresh: async () => Promise.resolve(false),
        },
      };
      const tokenSet: TokenSet = {
        expired: () => false,
        claims: () => ({ aud: 'aud', exp: 1, iat: 1, iss: 'iss', sub: 'sub' }),
      };
      console.log('CALL 0 BEFORE', spy.getCall(0));
      console.log(plugin['updateStateWithTokenSet']);
      plugin['updateStateWithTokenSet'](state, tokenSet);
      console.log('CALL 0 AFTER', spy.getCall(0));
      expect(spy.calledWith('mongodb-oidc-plugin:missing-id-token'));
      expect(spy.calledWith('this-is-bullshit'));
    });
    ...
    //     provider.getMongodbOIDCDBInfo().clientId
    //   );
    //   // specific to the no id_token case
    //   expect(logEmitSpy.calledWith('mongodb-oidc-plugin:missing-id-token'));
    // });
  });

  context.skip('with incomplete configuration', function () {
  ...
  context.skip('when an unusable redirect URL is provided', function () {
    beforeEach(function () {
      plugin = createMongoDBOIDCPlugin({
        ...defaultOpts,
        allowedFlows: ['auth-code', 'device-auth'],
  ...
  context.skip('when being aborted externally', function () {
  ...
  context.skip('when the server announces invalid configurations', function () {
  ...
  describe.skip('automaticRefreshTimeoutMS', function () {
  ...
  describe.skip('issuer URL validation', function () {
    beforeEach(function () {
      plugin = createMongoDBOIDCPlugin({
        ...defaultOpts,
      });
  ...
  describe.skip('Okta integration tests', function () {
  ...
  describe.skip('Azure integration tests', function () {
    let metadata: IdPServerInfo;
    let username: string;
    let password: string;
    let issuer: string;
